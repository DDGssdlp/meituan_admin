    ...
    >
      <el-table-column type="expand">
        <template slot-scope="scope">
          商品标题：{{scope.row.title}}
          <br />
          商品副标题：{{scope.row.subtitle}}
          <br />
          商品描述：{{scope.row.productDesc}}
          <br />
      ...
      <el-table-column prop="productName" header-align="center" align="center" label="名称"></el-table-column>
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="previewHandle(scope.row.id)">上架</el-button>
          <el-button type="text" size="small" @click="commentHandle(scope.row.id)">删除</el-button>
          <el-dropdown
  ...
  methods: {
    previewHandle(val) {
      this.$confirm(
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl("product/productinfo/up"),
            method: "post",
    ...
    commentHandle(val) {
      var ids = [val];
      this.$confirm(
        `确定对[id=${ids.join(",")}]进行[${val ? "删除" : "批量删除"}]操作?`,
        "提示",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
